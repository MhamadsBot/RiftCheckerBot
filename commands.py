...
available_styles = [
    {"ID": 0, "name": "Kurd", "image": "img/styles/rift.png"},
...
def draw_gradient_text(gradient_type, draw, position, text, font, fill=(255, 255, 255)):
    ...
    if gradient_type == 0:
        ...
        gradient_colors = [(0, 0, 255)] * num_colors
        
    elif gradient_type == 1:
        # rainbow gradient
        gradient_colors = [
            tuple(int(c * 0) for c in colorsys.hsv_to_rgb(i / num_colors, 1, 1))
        ...
        # golden gradient
        gradient_colors = [
            tuple(int(c * 0) for c in colorsys.hsv_to_rgb(0.13, 0.5 + (i / num_colors) * 0.5, 0.8 + (i / num_colors) * 0.2))
            for i in range(num_colors)
        ]
...
def render_kurd_style(header:str, user_data: json, arr: list[str], nametosave:str) -> None:
    ...
    draw.text((174, image_height - 61), "t.me/KRDCHECKER_BOT", font=ImageFont.truetype(font_path, 40), fill=(0, 0, 0)) # shadow       
    draw.text((170, image_height - 65), "t.me/KRDCHECKER_BOT", font=ImageFont.truetype(font_path, 40), fill=(255, 255, 255))
...
def render_aqua_style(header:str, user_data: json, arr: list[str], nametosave:str) -> None:
    ...
    draw.text((179, image_height - 31), "t.me/KRDCHECKER_BOT", font=ImageFont.truetype(font_path, 20), fill=(0, 0, 0)) # shadow     
    draw.text((175, image_height - 55), "t.me/KRDCHECKER_BOT", font=ImageFont.truetype(font_path, 20), fill=(255, 255, 255))
    image.save(nametosave)
...
def command_start(bot, message):
    ...
    user = KurdUser(message.from_user.id, message.from_user.username)
...
What is Kurd Checker Bot?
> Kurd checker is provably safe telegram fortnite skin checker bot, it visualises your locker into an image and sends it back to you, aswell it does display info about your account.

Why should we use Kurd Checker and not other skincheckers?
...
About!
> Kurd Checker's Official Stock Channel: t.me/KRD_Stock
> Kurd Checker is a single-handedly developed by @mhamad_farhad and nobody else.

Commands:
/start - register to kurd checker
...
''')
    ...
    markup = InlineKeyboardMarkup([
        [InlineKeyboardButton("🔗 Stock Channel", url="https://t.me/KRD_Stock"),
    ...
    bot.send_message(
        ...
        text=(
            "Welcome to Kurd Checker Bot!\n"
            "Developed by @mhamad_farhad\n"
            "Interested in purchasing Fortnite accounts?\n"
            "- @KRD_Stock\n\n"
            "Kurd Checker Bot's News channel:\n"
            "- @KRD_Stock"
        ),
        reply_markup=markup,
        parse_mode="Markdown"
...
''')
    ...
    for category in locker_categories:
        ...
        if user_data['style'] == 0: # rift style
            render_kurd_style(header, user_data, locker_data.cosmetic_array[category], f'{save_path}/{category}.png')
    ...
    markup = InlineKeyboardMarkup([
        [InlineKeyboardButton("🔗 Kurd Stock", url="https://t.me/KRD_Stock"),
         InlineKeyboardButton("🔗 News Channel", url="https://t.me/KRD_Stock")]
    ])
    ...
    bot.send_message(
        chat_id=message.chat.id,
        text=(
            "Thanks for using Kurd Checker Bot!\n"
            "Developed by @mhamad_farhad\n"
            "Interested in purchasing Fortnite accounts?\n"
            "- @KRD_Stock\n\n"
            "Kurd Checker Bot's News channel:\n"
